...
class MusicPlayer:
    ...
    def song_counter(self):# Total song present in the list
        messagebox.showinfo(
            "Song Counter",
            f"Total song in the list: {str(self.my_list_song.size())}",
        )
        ...
        try:
            ...
            if self.song_length == converted_time  and self.repeat_counter == 1:
                ...
                self.song_duration_bar.config(
                    text=f"Time is: {str(converted_time)} of {str(self.song_length)}"
                )
...
if __name__ == '__main__':
    ...
    window.iconbitmap("Generation-Music-Player-master\Pictures\music_icon.ico")
    window.geometry("828x515")
    window.maxsize(830,515)
    window.minsize(830,515)
    window.config(bg="#141414")

    backward_image_take = ImageTk.PhotoImage(Image.open('Generation-Music-Player-master\Pictures/backward.png'))
    ...
    play_image_take = ImageTk.PhotoImage(Image.open('Generation-Music-Player-master\Pictures\play.png').resize((40,40),Image.LANCZOS))
    ...
    pause_image_take = ImageTk.PhotoImage(Image.open('Generation-Music-Player-master\Pictures\pause.png'))
    ...
    stop_image_take = ImageTk.PhotoImage(Image.open('Generation-Music-Player-master\Pictures\stop_img_is.png').resize((40,40),Image.LANCZOS))
    ...
    forward_image_take = ImageTk.PhotoImage(Image.open('Generation-Music-Player-master\Pictures/forward.png'))
